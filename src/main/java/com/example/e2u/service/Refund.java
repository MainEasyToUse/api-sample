...
public class Refund {
  ...
  public String request(String payKey, PayReceipt payReceipt) throws Exception {
    ...
    String result = "";
    try {
      request(connection, new Gson().toJson(wrapData(payReceipt)));
  ...
  private Map<String, PayReceipt> wrapData(PayReceipt payReceipt) {
    Map<String, PayReceipt> data = new HashMap<>();

    data.put("refund", payReceipt);
